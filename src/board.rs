...
impl Board {
    ...
    pub fn dig(&mut self, x: i32, y: i32) -> String {
            ...
            || self.board[x as usize][y as usize] != Untouched.value()
        {
            ...
            let surrounding_bombs = self.get_adjacent_bombs(x as usize, y as usize);
            if surrounding_bombs != 0 {
    ...
    fn dig_recursively(&mut self, x: i32, y: i32) {
        ...
        while let Some(coordinate) = queue.pop_front() {
            let surrounding_bombs = self.get_adjacent_bombs(x as usize, y as usize);
            if self.board[coordinate[0] as usize][coordinate[1] as usize] == Flagged.value() {
                continue;
                ...
                if !self.bomb[coordinate[0] as usize][coordinate[1] as usize] {
                    self.board[coordinate[0] as usize][coordinate[1] as usize] =
                        NoNeighborBombs.value();
                }
                for dir in self.dirs {
                    let _x = dir[0] as i32 + coordinate[0];
                    let _y = dir[1] as i32 + coordinate[1];
        ...
        for dir in self.dirs {
                ...
                && self.board[_x as usize][_y as usize] != Flagged.value()
            {
                ...
                if self.board[_x][_y] == '1' {
                    ...
                    self.board[_x][_y] =
                        char::from_digit((self.board[_x][_y].to_digit(10).unwrap() - 1), 10)
                            .unwrap();
                }
            }
        }
